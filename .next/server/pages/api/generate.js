...
`,j=await h.chat.completions.create({model:"gpt-4o-mini",messages:[{role:"user",content:i}],temperature:1}),k=j?.choices?.[0]?.message?.content;if(!k)return b.status(200).json({email:`Subject: Quick idea for ${d}
...
Yours sincerely,  
[Your Name]`});return b.status(200).json({email:k})}catch(a){return console.error("❌ SERVER ERROR:",a),b.status(500).json({error:"Failed to generate email"})}}d()}catch(a){d(a)}})},7984:a=>{a.exports=import("openai")}};var b=require("../../webpack-api-runtime.js");b.C(a);var c=b.X(0,[169],()=>b(b.s=3362));module.exports=c})();